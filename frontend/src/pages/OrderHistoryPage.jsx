...
import { getOrderHistory } from '../components/GetOrSaveOrderHistory';

function OrderHistoryPage({userEmail}) {
    const navigate = useNavigate();
    const orders = useMemo(() => getOrderHistory().filter(order => order.user === userEmail), [userEmail]);
    return (
        ...
        <h1 className="text-4xl font-extrabold tracking-tight text-white mb-8 border-b border-gray-800 pb-4">
            Order History for <span className="text-cyan-400">{userEmail}</span>
        </h1>
        ...
        <button
            onClick={() => navigate('/')}
            className="mt-8 text-gray-400 hover:text-cyan-400 transition flex items-center space-x-1"
        >
            <ChevronLeft size={20} />
